...
async def run_async_bias_audit(application_id: int, company_id: int, db_session):
    ...
    company_id is used for tenant isolation and audit context.
    """
    try:
        import json
...
async def run_drift_check(company_id: int = None):
    """
    Run drift monitoring check across recent interviews.
    When company_id is provided, only checks interviews for that company.
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            query = db.query(Application).filter(
                Application.interview_state == "completed",
...
async def collect_calibration_samples(company_id: int = None):
    """
    Collect calibration samples from interviews that have human reviews.
    When company_id is provided, only collects samples for that company.
    """
    try:
        import json
...
async def run_score_recalibration(company_id: int = None):
    """
    Recalculate scores for recent interviews when scoring logic changes.
    When company_id is provided, only recalibrates scores for that company.
    """
    try:
        import json
                ...
                try:
                    ...
                    if abs(new_breakdown.final_score - old_score) > 2:
                        ScoringService.compute_final_score(
                            app,
                            db,
                            computed_by="recalibration",
                            override_rubric_score=new_breakdown.final_score,
                        )
                        recalibrated += 1
