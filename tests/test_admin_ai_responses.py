...
def _client(admin=True):
    ...
    (the only correct mechanism for overriding Depends()-wired
    functions after router registration).

    NOTE: this file was previously using
    ``monkeypatch.setattr(..., raising=False)`` which does NOT
    take effect once the route has been registered. Rewritten
    to use the proper FastAPI override pattern.
    """
...
class TestAdminAPIRBAC:
    ...
    def test_admin_announcements_forbidden(self, non_admin_client):
        # /api/v1/admin/announcements/active is intentionally public
        # (powers the public landing-page banner).
